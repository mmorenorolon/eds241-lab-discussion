...
summ(m1_ols, model.fit = F)
```
Intercept: Without treatment, we expect the number of treated piles to be ~78 on average.

treat: The treatment reduced the average number of trash piles by approximately  25 piles. 
...
post_treat_subset %>% 
  ...
  geom_histogram(binwidth = 5, , fill = "blue", color = "white", alpha = .7) + 
...
GLMs, specifically the "Poisson" model, since our variable is count data. 
...
summ(m2_poisson, model.fit = F)
```
Intercept: On average, the model estimates our control group to have around 78 waste piles.

Coefficient: The model estimates that treatment reduced waste by *32%*.
...
summ(m3_log, model.fit = F)
```
```{r}
# Intercept
exp(4.31)

# treated neighborhoods (B1)
((exp(-.42)-1)*100)
```
Intercept: On average, the number waste piles in the control group is 78.

Coefficient (treatment): After treatment, the number of waste piles has decreased by 35%. 

We can compare our observed data to simulated data based on the model we just created (`m3_log`).

```{r}
check_predictions(m3_log, verbose = F)
```
### Model Recap 
...
m3_change = (exp(-.42) - 1)*100 # % change LOG-OLS = -34.3
...
post_treat_subset %>% 
  ...
  tbl_summary(
    by = treat,
    statistic = list(all_continuous() ~ "{mean} ({sd})")) %>% modify_header(label~ "**Variable**") %>% 
...
The pre count variable shows that the treated group already has a lower average of waste piles.
...
```{r}
m4_control<- lm(log(waste_piles) ~
                  treat + log(pre_count_avg),
                data = post_treat_subset)

summ(m4_control, model.fit = F)
```
...
There is approximately a 1% change in the pre-count average. 

Let's compare our two log OLS Regression Models. 

```{r}
export_summs(m3_log, m4_control,
             model.names = c("Log OLS Regression", 
                             "Log OLS Regression with a control variable"),
                             statistics = "none")
```
```{r}
m3_change = (exp(-0.42) - 1)*100 ## % change LOG-OLS = -34.3

m4_change = (exp(-.27) -1)* 100
```
Intercept
