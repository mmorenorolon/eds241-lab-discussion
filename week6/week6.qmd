...
**RESPONSE:** Scenarios: seasonal viruses (flu), pollen/allergens, traffic patterns, human behavior
...
```{r}
options(noaakey = "QFaaqnsLnMNdvdKtvgbWPbwJLvAjZGXf")
```
...
precip_raw <- ghcnd_search(stationid = "USC00045933", var = "PRCP",
                           date_min = "2023-01-01", 
                           date_max = "2023-12-31") %>% 
  pluck("prcp") %>% 
  select(id, date, prcp) %>% 
...
trends <- gtrends(keyword = "asthma inhaler",
                  geo = "US-CA-803",
...
:::
  ...
  Our final dataset contains 53 observations. This is a relatively small sample for regression analysis, which means our estimates will have higher variance, larger standard errors, and lower statistical power. With so few observations, the results become sensitive to outliers, and weak‑instrument problems are amplified in the IV setting. As a result, both the OLS and IV estimates should be interpreted cautiously.
...
ggplot(data_weekly_joined, aes(x = precip_total, y = pm25_mean)) +
  ...
  geom_smooth(method = "lm", se = TRUE, color = "dodgerblue", linewidth = 1) +
  labs(
    x = "Weekly Total Precipitation (inches)",
    y = "Weekly PM2.5",
    title = "Precipitation vs. PM 2.5 (First Stage Analysis)"
  ) +
  theme_minimal()
...
**RESPONSE:** Even with month fixed effects, OLS is still biased because many short‑run environmental and behavioral factors jointly influence PM2.5 and respiratory illness. Month FE remove seasonal confounding, but not the week‑to‑week endogeneity that motivates the IV approach.  
...
**RESPONSE:** The coefficient on `precip_total` in the first stage is negative. That is consistent with expectations because more precipitation tends to wash out particulate matter (wet deposition), so higher weekly total precipitation should be associated with lower PM2.5 levels. Is precipitation a strong or weak instrument? The first-stage F‑statistic of about 3.16 is well below the common rule‑of‑thumb threshold of 10 for a strong instrument. That means precipitation is a weak instrument here.
...
```{r}
m3_ivreg <- ivreg(asthma_index ~ pm25_mean + factor(month(week_start)) | precip_total + factor(month(week_start)),
                  data = data_weekly_joined)
...
**RESPONSE:** One mechanism by which precipitation could directly affect asthma searches is mold growth, since wet conditions promote mold spores. In this case, rain reduces PM2.5 (first stage: negative coefficient), but it may also increase asthma symptoms (direct effect). This creates a negative correlation between PM2.5 and asthma that is not causal, making the IV estimate biased and inconsistent.
...
**RESPONSE:** We could add more climate variables like humidity, wind, etc. Also, increasing the sample size by prolonging the study period. More weeks or locations could increase statistical power.
...
**RESPONSE:** Another variable that might serve as an instrument is upwind wildfire smoke plumes, since smoke exposure could affect asthma through PM2.5, although not independently. 
...
**RESPONSE:** The 4 key assumptions required for the IV strategy (2SLS) are:
- Independence: precipitation must not be related to unobserved factors.
- Relevance: Cov(𝑍,𝑋) ≠ 0. Precipitation must meaningfully predict PM2.5. In our data, this is weak (F ≈ 3.16).
- Monotonicity: The instrument pushes the treatment in the same direction for everyone. More precipitation should never increase PM2.5 for some weeks and decrease it for others.
- Exclusion: 𝑍affects 𝑌only through𝑋. Precipitation must not directly affect asthma searches. This is questionable due to humidity, mold, and behavioral pathways.
