...
data_binned <- sim_data %>% 
  mutate(distance_bin = cut(distance, breaks = seq(-20, 20, by = 1),
                            include.lowest = TRUE)) %>% 
...
```{r}
rdd_lm <- ggplot(data_binned, aes(x = avg_distance, y = avg_deforest,
...
The lines indicate the deforestation rate within and outside protected areas. The trends reveal a 0.1 difference in deforestation rate between protected and unprotected areas.
...
summ(rdd_ols, model.info = FALSE, model.fit = FALSE, digits = 3)
```
...
**RESPONSE**: Protected areas have an average of 10% lower rate of deforestation. This is consistent with what the plot demonstrated. The interaction coefficient represents a 0.2% increase of sloped terrain in protected areas.
...
global_rdd <- rdrobust(
  ...
  covs = global_samp %>% select(slope_cat, water_cat, soil_cat, road_cat),
  c = 0, # cutoff at 0
  kernel = "triangular"
)
...
**RESPONSE**: Since the local polynomial regression sets the cutoff at 0, it provides a smaller estimate with a smaller bandwidth of data (-2 to 2) [BW Est. (h) = 2.207] compared to the previous model, which used a (-20, 20) bandwidth. This method is more reliable since the values are closer to the  reduces bias. 
...
**RESPONSE**: The key difference is that the local polynomial model is not linear, it follows the trend more closely than the linear model. Therefore, the local polynomial model captures the pattern in the data more accurately.
333
...
**RESPONSE**: There is higher discontinuity in for some countries in the simulated data than in the study (e.g. Indonesia and Brazil).
