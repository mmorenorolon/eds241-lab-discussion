...
LA_2019 <- read_csv(here::here("week3", "data",  "LA_2019.csv")) %>%
  filter(`Local Site Name` == "West Los Angeles") %>% 
  mutate(city = "LA", year = 2019)


LA_2020 <- read_csv(here::here("week3" ,"data", "LA_2020.csv")) %>% 
  filter(`Local Site Name` == "West Los Angeles") %>% 
  mutate(city = "LA", year = 2020)
...
weekly_la_phx <- did_data_az_ca %>% 
  ...
  group_by(week, city) %>% 
  summarize(
    mean_no2 = mean(daily_max_1_hour_no2_concentration, 
                    na.rm = TRUE, 
                    .groups = "drop")
  )
...
- From this figure, we can determine that this trend does not follow the parallel tendency assumption for a DD analysis.
...
NM_2019 <- read_csv(here::here("week3", "data", "nm_2019.csv")) %>% 
  mutate(state = "NM", year = 2019, 
      `County FIPS Code` = as.numeric(`County FIPS Code`))                                                                                   

NM_2020 <- read_csv(here::here("week3", "data", "nm_2020.csv")) %>%  
  mutate(state = "NM", year = 2020,                       `County FIPS Code`  = as.numeric(`County FIPS Code`))
...
- They share similar characteristics, population size and demographic, which might reflect similar trends. 
...
did_data_nm_tx <- did_data_nm_tx %>% 
  mutate(date_obj = mdy(date),
         ...
         #Post = after Mar 23
         is_post = ifelse(date_obj >= as.Date("2020-03-23"), 
                          1, 0)
         )
```
...
y_trt_post <- did_data_nm_tx %>% 
  filter(treated == 1, is_post == 1, year == 2020,
         month(date_obj) %in% c(2, 3, 4)) %>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

y_trt_pre <- did_data_nm_tx %>% 
  filter(treated == 1, is_post == 0, year == 2020,
         month(date_obj) %in% c(2, 3, 4)) %>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

y_con_post <- did_data_nm_tx %>% 
  filter(treated == 0, is_post == 1, year == 2020,
         month(date_obj) %in% c(2, 3, 4)) %>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

y_con_pre <- did_data_nm_tx %>% 
  filter(treated == 0, is_post == 0, year == 2020,
         month(date_obj) %in% c(2, 3, 4)) %>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

trt_effect <- (y_trt_post - y_trt_pre) - 
  (y_con_post - y_con_pre)
...
- This value indicates a drop in NO2 concentrations during that time period.
:::{.callout-warning}
...
- Texas males a more compelling counterfactual, but since we see week-to-week fluctuations in a three-month period, it is harder to discern whether this is a compelling counterfactual. 
...
There's 22% probability that we would have observed these results. 
...
Our counterfactual is El Paso would have served as the counterfactual to Dona Ana. Since the dust storm occurs in both counties, they would have been affected equally.   
...
- Our DiD estimate would be smaller, therefore, we would see more parallel trends. 