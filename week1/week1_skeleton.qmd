...
class_data <- read_csv("https://raw.github.com/garberadamc/W26-Policy-Eval/main/course-materials/labs/data/W26_class_survey.csv") %>% 
  ...
  mutate(random_groups = sample(rep(c("control", "treatment"), 
  each = n()/2)))
...
```{r}
# calculate means and look at the difference between means
...
balance_summary <- class_data %>% 
  gtsummary::tbl_summary(
    ...
    #syntax explanation: parentheses around {sd} will show the standard deviations on the table for each statistic in parentheses
...
```{r}
tbl_2way %>% 
  #coming from a dataframe use gt(), otherwise, as_gt()
  gt() %>% 
  cols_label(
    bol = "Bolærne",
    flo = "Kvernskjær",
...
tbl_3way <- lobsters %>% 
  ...
  summarise(total_cpue = sum(cpue, na.rm = T),
            .groups = "drop") %>% 
  # Pivot to create column for each trtmt/control group within each region
...
fancy_table <- tbl_3way %>% 
  ...
  tab_header(title = "Eurpoean Lobster Catch by Year, Region, Treatment",
...
table_w_plots <- lobsters %>% 
  ...
  summarise(
    total_cpue = sum(cpue, na.rm = T),
    dist_cpue = list(cpue), # store all individual cpue values as sa list for ea/ year
    .groups = "drop"
  ) %>% 
...
Air quality / public health issues prompted the study 
2. How were the treatment and control groups formed? 
Out of the assigned 44 groups in Nansana  Municipality, half werre assigned to the treatment and the other to the control group. 

3. Discuss the matched-pairs design of the study. How were neighborhoods paired? Create a diagram if helpful!

The researchers assigned half of the pairs to the social competition treatment. 

4. Was the social competition strategy effective?

The strategy was effective, where the study found a 24% reduction in informal waste burning in the treated group. 

5. What was the primary metric for assessing the impact of the social competition?

The primary metric is the count of waste piles showing signs of recent burning around multiple audits.
